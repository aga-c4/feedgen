...
$feedConfArr = array(
    ...
    */
    ...
    "name_replace" => null, //Строки замены имени типа array("строка1"=>"замена","строка2"=>"замена2").
    "model_replace" => null, //Строки замены модели типа array("строка1"=>"замена","строка2"=>"замена2").
    ...
    "descr_dop_top" => false, //Текст, который будет добавлен в начало description. Поддерживает формат массива с условиями рутовых категорий. 
    "descr_dop" => false, //Текст, который будет добавлен в конец description. Поддерживает формат массива с условиями рутовых категорий.
    ...
    "price_koef" => false, //Коэффициент, который применяется к цене. Поддерживает формат массива с условиями рутовых категорий.
    ...
    "use_gross_sw" => false, //Получить вес и размер БРУТТО - влияет на забор размеров и веса в trait CustomFeedgenMethods
    "use_netto_sw" => false, //Получить вес и размер НЕТТО - влияет на забор размеров и веса в trait CustomFeedgenMethods
    ...
    "gross_cat_def" => false, //Дефолтовые значения размеров/веса для корневых категорий каталога БРУТТО
    "netto_cat_def" => false, //Дефолтовые значения размеров/веса для корневых категорий каталога НЕТТО
    "gross_required" => false, //Блокировать вывод позиций по которым нет размеров БРУТТО и веса
    "netto_required" => false, //Блокировать вывод позиций по которым нет размеров НЕТТО и веса

    "use_condition_type" => false, //Выводить теги состояния в <condition...>
    ...
    "minpriceto" => false, //Если больше 0, то в случае если цена меньше этого значения, то подставляем это значение. Иногда требуют цены не ниже определенных (авито например для доставки).
    "use_maxprice" => false, //Использовать максимальную цену для ограничения вывода в фид
    "maxprice" => 100000, //Размер максимальной цены для вывода в фид, если не задано, то берем из общего конфига
    ...
    "reg_vals" => array(//Региональные настройки
        ...
        "default" => array (//По-умолчанию - используется, если не определено иное для региона.
            ...
            "domain_img" => null, //Домен по-умолчанию для изображений (если не задано, то берем из основноо конфига). Может применяться массив по рутовым категориям.
            ...
            "delivery_options" => array(
                "main" => array(
                    ...
                    'days_instock_max' => 7, //Максимальное время заказа товара в наличии (можно диапазон 1-3 и т.д.).
